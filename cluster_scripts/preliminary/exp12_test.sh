#!/bin/bash
#SBATCH --job-name=exp12_test
#SBATCH --partition=test
#SBATCH --gres=gpu:1
#SBATCH --time=02:00:00
#SBATCH --output=/networkhome/WMGDS/souval_g/neuRAWns_mmdet/cluster_scripts/logs_test/exp12_test_%j.out
#SBATCH --error=/networkhome/WMGDS/souval_g/neuRAWns_mmdet/cluster_scripts/logs_test/exp12_test_%j.err
...
# ── Test ──────────────────────────────────────────────────────
echo "=== Starting exp12 test ==="
cd /networkhome/WMGDS/souval_g/neuRAWns_mmdet
export PYTHONPATH="${PYTHONPATH}:$(pwd)"

# !! UPDATE THIS: replace epoch_X with the actual best epoch number
# Run: ls work_dirs/exp12/best_coco_bbox_mAP_*.pth
CHECKPOINT=/networkhome/WMGDS/souval_g/neuRAWns_mmdet/work_dirs/exp12/best_coco_bbox_mAP_epoch_5.pth

mim test mmdet configs/experiments/exp12_test.py     \
        --launcher none     \
        --checkpoint $CHECKPOINT    \
        --work-dir /networkhome/WMGDS/souval_g/neuRAWns_mmdet/work_dirs/exp12_test     \
        --cfg-options         test_dataloader.dataset.data_root=/networkhome/WMGDS/souval_g/data/ROD/yolo/    \
