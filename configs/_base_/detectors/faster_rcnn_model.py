# ─────────────────────────────────────────────────────────────
# _base_/detectors/faster_rcnn_model.py
# Faster R-CNN R50-FPN model dict ONLY, named `faster_rcnn_model`.
# 80-class COCO head (frozen, loaded from COCO ckpt via wrapper).
# Standard mmdet COCO normalisation (RGB-order mean, bgr_to_rgb=True).
# init_cfg=None on the backbone so building does NOT trigger a
# torchvision download — the COCO checkpoint overwrites it anyway.
# ─────────────────────────────────────────────────────────────
faster_rcnn_model = dict(
    type='FasterRCNN',
