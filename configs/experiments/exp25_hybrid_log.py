# ─────────────────────────────────────────────────────────────
# experiments/exp25_hybrid_log.py   [AUTO-GENERATED Phase-2 grid]
# Module:        LocalHybrid
# Normalisation: NormaliseLog  (Option-2 fixed-log: pipeline pre-compresses HDR)
...
exp_name = 'exp25_hybrid_log'

auto_scale_lr = dict(enable=True, base_batch_size=64)

# ── Pipeline ──────────────────────────────────────────────────
# ROD / Sony IMX490 sensor constants (from dataset profiling)
norm = dict(type='NormaliseLog', black_level=8.5, white_level=4015028.0)
...
vis_backends = [
    ...
    dict(type='WandbVisBackend', init_kwargs=dict(
        ...
        config=dict(
            phase=2,
            input='packed_4ch',
            preprocessor='LocalHybrid',
            normalisation='NormaliseLog',
            knot_spacing='log',
            grid='8x8',
            out_scale=255.0,
            detector='RTMDet-S',
