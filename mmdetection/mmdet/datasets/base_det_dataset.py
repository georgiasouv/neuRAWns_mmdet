...
class BaseDetDataset(BaseDataset):
    ...
    def full_init(self) -> None:
        ...
        print(f"AFTER load_data_list: {len(self.data_list)} items")

        # get proposals from file
        if self.proposal_file is not None:
            self.load_proposals()
            print(f"AFTER load_proposals: {len(self.data_list)} items")
        # filter illegal data, such as data that has no annotations.
        self.data_list = self.filter_data()
        print(f"AFTER filter_data: {len(self.data_list)} items")


        # Get subset data according to indices.
        if self._indices is not None:
            self.data_list = self._get_unserialized_subset(self._indices)
            print(f"AFTER get_unserialized_subset: {len(self.data_list)} items")


        # serialize data_list
        if self.serialize_data:
            print(f"BEFORE serialize: {len(self.data_list)} items")
