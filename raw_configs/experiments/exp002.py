...
custom_imports = dict(
    imports=['modules.exp002_learnable_isp_v1',
             'raw_preprocessing.modules.raw_resnet_backbone',
             'raw_preprocessing.datasets.pipelines.loading'],
...
test_evaluator = dict(
    type='CocoMetric',
    ann_file=data_root + 'json_raw/test.json',  # Match test annotations
    metric='bbox',
    format_only=False)
...
model = dict(
    ...
    # Data preprocessor - NO normalisation (your module handles it)
    data_preprocessor=dict(
        type='DetDataPreprocessor',
        mean=[0.0, 0.0, 0.0],      # No mean subtraction
        std=[1.0, 1.0, 1.0],       # No std normalisation
        bgr_to_rgb=False,          # Your preprocessor outputs RGB
        pad_size_divisor=32        # Pad to multiples of 32 for FPN
    ),
    
    # YOUR CUSTOM BACKBONE
    backbone=dict(
        type='RAW_ResNet',  # Your registered class name
        
        # Configuration for your preprocessing module
        preprocess_cfg=dict(
        ...
        # ResNet-50 configuration (passed via **resnet_kwargs)
        depth=50,
        num_stages=4,
